...
const toSIUnits = (number: number): string => {
...
function getNetworkLoad(): string {
  ...
  const interfaces = exec(`ls -w1 ${path}`).split("\n");
  ...
  return `↑${toSIUnits(speedOut)}B/s\n↓${toSIUnits(speedIn)}B/s`;
}

export const speeds = Variable<string>("↑0B/s ↓0B/s");

speeds.poll(1000, getNetworkLoad);
